...
namespace PLC通讯基础控件项目.控件类基.控件文本键盘
{
    public partial class keyboard_Hex:UIForm
    {
        ...
        private string Button_text_add(object send)//实现刻录字符
        {
            ...
            if (textBox.ReadWritePLC.ToString() == "HMI") return data;//用户使用了宏指令内部寄存器直接刻录
        ...
        private string[] Constraints_data(string Name)//实现约束最小值--最大值
        {
            ...
            switch (Name)
            {
                case "Hex_16_Bit":
                    //最大值
                    if (textBox.NumericaldigitMax >= 4)
                    ...
                    case "Hex_32_Bit":
                    //最大值
                    if (textBox.NumericaldigitMax >= 8)
